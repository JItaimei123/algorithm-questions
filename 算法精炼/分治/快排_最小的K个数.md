...
**补充：为什么 31 行用 `return` 而不用 `qsort(arr,l,right-1,k)`？**

如果改为 `qsort(arr, l, right-1, k)`，你会进入无限递归，因为 `right-1` 和 `l` 之间的区间是 `[l, right-1]`，而这个区间已经包含了所有小于等于 `pivot` 的元素，且它们的相对位置已经确定，完全满足"最小的 k 个数已经在前 k 个位置"的条件。你不需要再排序这个区间，因为它已经排好了。`return` 就是告诉函数"已经完成了"，结束递归，不要再往下走。如果你强行再调用一次 `qsort`，它会把已经排好的区间再排一遍，导致死循环和栈溢出。
